...
`project-brain` is a model-independent project brain for AI coding editors.

It stores durable project understanding beside source code so Codex, Cursor, Claude, Gemini, and future tools can reconstruct intent, architecture, terminology, current focus, behavior contracts, constraints, decisions, and failed experiments before changing code.

## Getting Started

Install the CLI globally directly from GitHub, then initialize it in your project:
...
`project-brain init [directory]` creates the `PROJECT_BRAIN.md` entrypoint, an `AGENTS.md` adapter if missing, a `PROJECT_BRAIN_BOOTSTRAP.md` prompt file, and a neutral `project-brain/` directory structure.

*(Note: Initialization refuses to replace managed files by default. Use `project-brain init --force` if you want to back up and overwrite existing managed files.)*

### Bootstrapping Your Project

Generated project facts are marked `unverified` initially. To automatically verify and populate your brain with your project's context, tell your AI editor:

> *"Read PROJECT_BRAIN_BOOTSTRAP.md and execute every step completely."*

The AI will scan your codebase and intelligently fill out all the `project-brain` markdown files.
...
When you start a new chat with your AI, use this boot prompt once:
...
After booting, simply give your normal requests:
...
Editors that automatically discover `AGENTS.md` (like Codex) will boot correctly for every request.

For editors that do not discover it automatically, configure one permanent project instruction (e.g., in `.cursorrules` or Claude's custom instructions):
...
```text
project-root/
├── PROJECT_BRAIN.md               # Universal entrypoint
├── AGENTS.md                      # Optional thin editor adapter
├── PROJECT_BRAIN_BOOTSTRAP.md     # One-time initialization prompt
├── project-brain/
│   ├── MANIFEST.yaml              # Boot and on-demand loading
│   ├── purpose/
│   ├── glossary/
│   ├── architecture/
│   ├── features/
